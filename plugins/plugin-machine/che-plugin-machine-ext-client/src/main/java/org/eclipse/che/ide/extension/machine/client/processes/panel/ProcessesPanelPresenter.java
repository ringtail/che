...
//        addMachineNode(event.getMachine());
     ...
     */
    @Override
    public void onAddTerminal(@NotNull final MachineEntity machine) {
    ...
    @Override
    public void onPreviewSsh(String machineName) {
        final ProcessTreeNode machineTreeNode = findProcessTreeNodeById(machineName);
        if (machineTreeNode == null) {
            return;
        }

        final MachineEntity machine = (MachineEntity)machineTreeNode.getData();

        final OutputConsole defaultConsole = commandConsoleFactory.create("SSH");
        addCommandOutput(machineName, defaultConsole);

        final String sshServiceAddress = getSshServerAddress(machine);
        String machineHost = "";
     ...
     *
     * @param machineName
     *         name of the machine in which the command will be executed
     * @param outputConsole
     *         the console for command output
     */
    public void addCommandOutput(String machineName, OutputConsole outputConsole) {
        final ProcessTreeNode machineTreeNode = findProcessTreeNodeById(machineName);
        if (machineTreeNode == null) {
        ...
        String commandId;
        final String outputConsoleTitle = outputConsole.getTitle();
        final ProcessTreeNode processTreeNode = getProcessTreeNodeByName(outputConsoleTitle, machineTreeNode);
        ...
        final ProcessTreeNode commandNode = new ProcessTreeNode(COMMAND_NODE,
                                                          machineTreeNode,
                                                          outputConsoleTitle,
    ...
    private ProcessTreeNode findProcessTreeNodeById(String id) {
        Log.error(getClass(), "!!! find  root node " + rootNode.getChildren());
        for (ProcessTreeNode processTreeNode : rootNode.getChildren()) {
            Log.error(getClass(), "!!! find  id " + id + " /// nodeId " + processTreeNode.getId());
    ...
    private ProcessTreeNode addMachineNode(MachineEntity machine) {
        ...
        Log.error(getClass(), "=== add machine node");
        rootNodes.add(machineNode);

        if (!consoles.containsKey(machineName)) {
            addOutputConsole(machineName, machineNode, commandConsoleFactory.create(machineName), true);
        }
    ...
    private ProcessTreeNode provideMachineOutputNode(final String machineName) {
        ...
        final ProcessTreeNode machineOutputNode = new ProcessTreeNode(MACHINE_OUTPUT_NODE, rootNode, machineName, null, new ArrayList<ProcessTreeNode>());
        machineOutputNode.setRunning(true);
        machineNodes.put(machineName, machineOutputNode);
        rootNodes.add(machineOutputNode);
        view.setProcessesData(rootNode);

        final OutputConsole outputConsole = commandConsoleFactory.create(machineName);
    ...
    public void fetchMachines() {
        ...
        workspaceService.getWorkspace(appContext.getWorkspaceId()).then(new Operation<WorkspaceDto>() {
            ...
            public void apply(WorkspaceDto workspace) throws OperationException {
                ...
                Log.error(getClass(), "=== clear nodes");
                machineNodes.clear();
                rootNode.getChildren().clear();
                view.setProcessesData(rootNode);

                ProcessTreeNode machineToSelect = null;
                MachineEntity devMachine = getDevMachine(machines);
                if (devMachine != null) {
    ...
    private MachineEntity getDevMachine(List<MachineEntity> machines) {
        for (MachineEntity machine : machines) {
            if (machine.isDev()) {
                return machine;
            }
    ...
    private void restoreState(final MachineEntity machine) {
        Log.error(getClass(), "restore state");
            ...
            public void apply(List<MachineProcessDto> arg) throws OperationException {
                for (MachineProcessDto machineProcessDto : arg) {
                    ...
                    if (type != null ) {
                        ...
                        addCommandOutput(machine.getDisplayName(), console);
                    }
    ...
    public void printMachineOutput(String machineName, String text) {
        final OutputConsole console = consoles.get(machineName);
